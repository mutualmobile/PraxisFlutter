import 'package:clean_architecture/src/viewmodel.dart';
...
/// page. The [ResponsiveViewState] requires a [ViewModel] to handle its events and provide its data.
///
...
/// in a widget to grant easy access to the [ViewModel], which could then use it to display
/// snackbars, dialogs, and so on.
///
/// The [ResponsiveViewState] lifecycle is also handled by the [ViewModel].
///
...
abstract class ResponsiveViewState<Page extends View, VM extends ViewModel>
    extends ViewState<Page, VM> {
  ResponsiveViewState(VM viewModel) : super(viewModel);
...
/// page. The [ViewState] requires a [ViewModel] to handle its events and provide its data.
///
/// The [ViewState] also has a default [globalKey] that can be used inside its `build()` function
/// in a widget to grant easy access to the [ViewModel], which could then use it to display
/// snackbars, dialogs, and so on.
///
/// The [ViewState] lifecycle is also handled by the [ViewModel].
/// ```dart
...
///
/// If a [RouteObserver] is given to the [View], it is used to register its [ViewModel] as
