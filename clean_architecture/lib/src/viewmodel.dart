...
/// A Clean Architecture [ViewModel]. Should be aggregated within a `ViewState` or
/// a `View`. However, it is preferable to be contained inside the `View` for readability
/// and maintainability.
///
/// The [ViewModel] handles the events triggered by the `View`. For example, it handles
...
abstract class ViewModel
  ...
  /// Unmounts the [ViewModel] from the `View`. Called by the `View` automatically.
  ...
  ///
  /// The usage of [ViewModel.getContext] will be impossible here. Since the context will be null after the widget be removed from the widget
  /// tree.
  @mustCallSuper
  void onDisposed() {
    assert(_globalKey.currentContext == null,
        ...
        For example:
        If this does not resolve for you, please open an issue at `https://github.com/ShadyBoukhary/clean_architecture` describing 
     the error.''');
    dispose();
  }
